...
final class TrajetsController extends AbstractController
{
                ...
                content: new OA\JsonContent(
                    ...
                    items: new OA\Items(
                        ...
                        properties: [
                            ...
                            new OA\Property(property: 'places', type: 'integer', example: 4),
                            new OA\Property(property: 'places_restantes', type: 'integer', example: 3),
    ...
    public function index(TrajetsRepository $trajetsRepository): JsonResponse
    {
        try {
            ...
            $data = array_map(function (Trajets $trajet) {
                // Calculer les places restantes
                $placesRestantes = $trajet->getPlaces() - $trajet->getPassager()->count();
    ...
    #[Route('/', name: 'app_trajets_create', methods: ['POST'])]
    #[OA\Post(
        ...
        requestBody: new OA\RequestBody(
            content: new OA\JsonContent(
                ...
                required: ['conducteur_id', 'depart', 'arrive', 'date', 'duree', 'prix', 'places', 'voiture_id']
            )
        ),
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'status', type: 'string', example: 'Trajet créé avec succès'),
                        new OA\Property(property: 'id', type: 'integer', example: 1)
                    ]
                )
            ),
    ...
    ): JsonResponse
    {
        try {
            ...
            $requiredFields = ['conducteur_id', 'depart', 'arrive', 'date', 'duree', 'prix', 'places', 'voiture_id'];
            $missingFields = [];
            ...
            return $this->json([
                'status' => 'Trajet créé avec succès',
                'id' => $trajet->getId()
    ...
    #[OA\Get(
        ...
        parameters: [
            new OA\Parameter(name: 'id', in: 'path', schema: new OA\Schema(type: 'integer'), required: true, description: 'ID du trajet', example: 1)
        ],
        responses: [
            new OA\Response(
                response: 200,
                description: 'Trajet retourné',
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'id', type: 'integer', example: 1),
            ...
            new OA\Response(response: 404, description: 'Trajet non trouvé')
        ]
    )]
    ...
    #[Route('/{id}', name: 'app_trajets_update', methods: ['PUT'])]
    #[OA\Put(
        ...
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'status', type: 'string', example: 'Trajet mis à jour avec succès'),
                        new OA\Property(property: 'id', type: 'integer', example: 1)
                    ]
                )
            ),
    ...
    ): JsonResponse
    {
        try {
            ...
            $trajet->setPlaces($data['places'] ?? $trajet->getPlaces());
            ...
            return $this->json([
                'status' => 'Trajet mis à jour avec succès',
                'id' => $trajet->getId()
            ]);
        } catch (\Exception $e) {
            return $this->json(
    ...
    #[Route('/{id}', name: 'app_trajets_delete', methods: ['DELETE'])]
    #[OA\Delete(
        ...
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'status', type: 'string', example: 'Trajet supprimé avec succès'),
                        new OA\Property(property: 'id', type: 'integer', example: 1)
                    ]
                )
            ),
    ...
    public function delete(Trajets $trajet, EntityManagerInterface $em): JsonResponse
    {
        try {
            $em->remove($trajet);
            $em->flush();
            ...
            return $this->json([
                'status' => 'Trajet supprimé avec succès',
                'id' => $trajet->getId()
            ]);
        } catch (\Exception $e) {
            return $this->json(
